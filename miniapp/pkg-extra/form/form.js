  ...
  _loadTemplate(id) {
    ...
    formService.getFormTemplate(id)
      .then((res) => {
        ...
        let fields = []
        try {
          const rawFields = res.fields || res.fieldList || []
          fields = typeof rawFields === 'string' ? JSON.parse(rawFields || '[]') : rawFields
          if (!Array.isArray(fields)) fields = []
        } catch (e) {
          fields = []
        }
        // 后端契约字段为 field_key；端上统一归一成 key，避免空表/无法录入
        fields = fields.map((field, index) => {
          const key = field.key || field.field_key || field.name || `field_${index + 1}`
          return {
            ...field,
            key,
            name: field.name || key,
            type: field.type || field.field_type || 'text',
            label: field.label || field.title || `字段${index + 1}`,
            required: field.required !== false && field.required !== 0,
          }
        })
        if (!fields.length) {
          wx.showToast({ title: '表单暂无字段', icon: 'none' })
        }
        // 初始化表单值
        const formValues = {}
        ...
        const title = res.title || res.name || '表单填写'
        this.setData({
          templateTitle: title,
