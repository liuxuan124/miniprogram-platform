// pkg-trade/order-list/order-list.js — 我的订单列表
// 按状态筛选、下拉刷新、上拉加载
  ...
  _loadOrders(reset = false) {
    ...
    return orderService.getOrderList(params)
      .then((res) => {
        ...
        const total = (res && (res.total != null ? res.total : list.length)) || 0
      ...
      .catch(() => {
        const empty = !this.data.orders || this.data.orders.length === 0
